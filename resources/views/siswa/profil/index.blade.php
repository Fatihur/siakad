...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="card p-6">
        <div class="text-center">
            @if($siswa && $siswa->foto)
                <img src="{{ Storage::url($siswa->foto) }}" alt="Foto" class="w-24 h-24 rounded-full mx-auto mb-4 object-cover">
            @else
            ...
            @endif
            <h3 class="text-xl font-semibold text-[#1C2434]">{{ $siswa->nama ?? $user->name }}</h3>
            <p class="text-[#64748B]">NIS: {{ $siswa->nis ?? '-' }}</p>
        ...
        <div class="space-y-3 text-sm">
            <div class="flex justify-between">
                <span class="text-[#64748B]">Kelas</span>
                <span class="font-medium">{{ $siswa->kelas->nama ?? '-' }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-[#64748B]">Jurusan</span>
                <span class="font-medium">{{ $siswa->kelas->jurusan->nama ?? '-' }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-[#64748B]">Jenis Kelamin</span>
                <span class="font-medium">{{ $siswa->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</span>
            </div>
            @if($siswa->telepon)
            <div class="flex justify-between">
                <span class="text-[#64748B]">Telepon</span>
                <span class="font-medium">{{ $siswa->telepon }}</span>
            </div>
            @endif
        </div>
        @endif
        ...
        <form action="{{ route('siswa.profil.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" value="{{ $siswa->nama ?? $user->name }}" class="form-input bg-gray-100" disabled>
                    <p class="text-xs text-[#64748B] mt-1">Nama tidak dapat diubah</p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" value="{{ $siswa->nis ?? '-' }}" class="form-input bg-gray-100" disabled>
                </div>
                
                <div>
                    <label class="form-label">Telepon</label>
                    <input type="text" name="telepon" value="{{ old('telepon', $siswa->telepon ?? '') }}" class="form-input @error('telepon') border-meta-1 @enderror">
                    @error('telepon')
                        <p class="text-meta-1 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat" rows="3" class="form-input @error('alamat') border-meta-1 @enderror">{{ old('alamat', $siswa->alamat ?? '') }}</textarea>
                    @error('alamat')
                        <p class="text-meta-1 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
