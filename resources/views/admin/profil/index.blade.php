...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="card p-6">
        <div class="text-center">
            ...
            <h3 class="text-xl font-semibold text-[#1C2434]">{{ $user->name }}</h3>
            <p class="text-[#64748B] capitalize">{{ $user->role }}</p>
        ...
        <form action="{{ route('admin.profil.update') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" class="form-input @error('name') border-meta-1 @enderror" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" value="{{ $user->username }}" class="form-input bg-gray-100" disabled>
                    <p class="text-xs text-[#64748B] mt-1">Username tidak dapat diubah</p>
                </div>
                
                <div>
                    <label class="form-label">Role</label>
                    <input type="text" value="{{ ucfirst($user->role) }}" class="form-input bg-gray-100" disabled>
                </div>
            </div>
