...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-[#64748B] mb-2">
        <a href="{{ route('admin.siswa.index') }}" class="hover:text-primary">Siswa</a>
        <i class="fas fa-chevron-right text-xs"></i>
        <span>{{ $isEdit ? 'Edit' : 'Tambah' }}</span>
    </div>
    <h2 class="text-2xl font-bold text-[#1C2434]">{{ $isEdit ? 'Edit Siswa' : 'Tambah Siswa Baru' }}</h2>
    <p class="text-[#64748B]">{{ $isEdit ? 'Perbarui data siswa' : 'Tambahkan data siswa baru' }}</p>
</div>

<div class="card max-w-4xl">
    <form action="{{ $isEdit ? route('admin.siswa.update', $data) : route('admin.siswa.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="alert alert-info">
                ...
                <span>Password default untuk siswa baru: <strong>password123</strong></span>
            </div>
            @endif
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="form-label">NIS <span class="text-meta-1">*</span></label>
                        <input type="text" name="nis" value="{{ old('nis', $isEdit ? $data->nis : '') }}" required class="form-input" placeholder="Nomor Induk Siswa">
                    </div>
                    <div>
                        <label class="form-label">NISN</label>
                        <input type="text" name="nisn" value="{{ old('nisn', $isEdit ? $data->nisn : '') }}" class="form-input" placeholder="Nomor Induk Siswa Nasional">
                    </div>
                    <div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $isEdit ? $data->nama : '') }}" required class="form-input" placeholder="Nama lengkap siswa">
                    </div>
                    <div>
            ...
            <div>
                <h4 class="text-sm font-semibold text-[#1C2434] uppercase tracking-wider mb-4">Kontak & Wali</h4>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="form-label">Telepon Siswa</label>
                        <input type="text" name="telepon" value="{{ old('telepon', $isEdit ? $data->telepon : '') }}" class="form-input" placeholder="08xxxxxxxxxx">
                    </div>
                    <div>
                        <label class="form-label">Nama Wali</label>
                        <input type="text" name="nama_wali" value="{{ old('nama_wali', $isEdit ? $data->nama_wali : '') }}" class="form-input" placeholder="Nama orang tua/wali">
                    </div>
                    <div>
                        <label class="form-label">Telepon Wali</label>
                        <input type="text" name="telepon_wali" value="{{ old('telepon_wali', $isEdit ? $data->telepon_wali : '') }}" class="form-input" placeholder="08xxxxxxxxxx">
                    </div>
                </div>
            </div>
        ...
        <div class="px-6 py-4 bg-[#F9FAFB] border-t border-stroke flex flex-col sm:flex-row gap-3">
            ...
            <a href="{{ route('admin.siswa.index') }}" class="btn btn-outline">
